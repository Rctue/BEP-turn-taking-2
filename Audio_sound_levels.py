...
class AudioRecorder:
    def __init__(self, input_device = 39, format=pyaudio.paInt16, channels=1, rate=44100, chunk=1024, verbose = False):
        ...
        self.CHANNELS = 1 # changed to one due to the audio interface which could handle stereo
        ...
        # Peak filtering
        self.peak_counter = 0
        self.min_peak_frames = 5   # require 2 frames above threshold (100ms)
    ...
    def calibrate(self):
        ...
        while not done:
            ...
            if speech < silence*3: # 2 would mean that max speech level is equal to median silence level, so better 3
                ...
                self.speech_threshold = silence + (speech - silence) * 0.7
    ...
    def record_audio(self):
        while self.is_recording:
            try:
                ...
                print(f"Audio read error on {self.device['name']}: {e}")
                continue

            self.frames.append(data)

            # 1) raw RMS first
            rms_raw = self.calculate_rms(data)

            # loud noises filtering
            if len(self.rms_data) > 1:
                prev = self.rms_data[-1]
                diff = abs(rms_raw - prev)
                if diff > 3:  
                    continue  
            ...
            if rms_value > self.speech_threshold:
                self.peak_counter += 1
                if self.peak_counter >= self.min_peak_frames:
